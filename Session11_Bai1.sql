...
/*
Giải thích 
- Do cú pháp yêu cầu bắt buộc truyền vào giá trị IN p_appointment_id INT 
- Khi truyền ID rỗng câu UPDATE vẫn chạy nhưng vẫn bị ảnh hưởng
- Nên hệ thống sẽ không cảnh báo người này có lịch hẹn chưa
*/
...
DELIMITER //
CREATE PROCEDURE CancelAppointment (IN p_appointment_id INT, OUT p_result INT)
BEGIN
    DECLARE v_exists INT;
    SELECT count(appointment_id) INTO v_exists 
    FROM Appointments 
    WHERE appointment_id = p_appointment_id;
...
CALL CancelAppointment(104, @result);
SELECT @result AS findID;
-- id 104 pending -> cancel
