...
-- Phần A: Phân tích
-- Lỗi nhập số lượng kho âm
CALL AddInventory(10, -200);
/*
Giải thích
- Dòng code stock_quantity = stock_quantity + p_quantity nếu như nhập 1 số âm thì sẽ thành hàng hóa âm
- Nên khi UPDATE số âm sẽ làm tổn thất hàng hóa kho
*/
...
BEGIN
    IF p_quantity > 0 THEN
        ...
        SET stock_quantity = stock_quantity + p_quantity
        WHERE item_id = p_item_id;
        SELECT 'Cập nhật thành công' AS result;
    ELSE
        SELECT 'Số lượng kho ko hợp lệ' AS result;
    END IF;
END //
DELIMITER ;
...
-- id 10 được thêm 50 hàng
SELECT item_id, item_name, stock_quantity FROM Inventory