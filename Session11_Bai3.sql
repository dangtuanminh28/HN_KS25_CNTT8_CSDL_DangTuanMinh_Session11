...
- p_patient_type VARCHAR(20)	(Loại đối tượng)
- p_total_cost DECIMAL(18,2)	(Tổng phí)

Output
- result_amount DECIMAL(18,2) (số tiền phải thu)
- result_message VARCHAR(20) (Thông báo trạng thái)

Các bước thực hiện
1. Kiểm tra: nếu ID bệnh nhân không tồn tại thì hệ thống báo lỗi.
2. Kiểm tra: nếu chi phí gốc thì hệ thống báo lỗi chi phí không hợp lệ
3. Tính toán: áp dụng giảm giá dựa vào loại đối tượng 
	20% cho BHYT 
	90% cho VIP
	100% cho THUONG
4. Cập nhật: lưu vào bảng Patient_Invoices và dùng SELECT in kết quả.
*/

DROP PROCEDURE IF EXISTS CalculatePatientInvoice;

DELIMITER //
CREATE PROCEDURE CalculatePatientInvoice (
    IN p_patient_id INT,
    IN p_patient_type VARCHAR(20),
    IN p_total_cost DECIMAL(18,2)
)
BEGIN
    DECLARE v_amount_to_pay DECIMAL(18,2);
    DECLARE v_message VARCHAR(255);
    DECLARE v_patient_exists INT;

-- Kiểm tra id bệnh nhân
    SELECT COUNT(patient_id) INTO v_patient_exists 
    FROM Patients 
    WHERE patient_id = p_patient_id;

    IF v_patient_exists = 0 THEN
        SET v_amount_to_pay = 0;
        SET v_message = 'Id bệnh nhân này không tồn tại';

    -- Trường hợp âm
    ELSEIF p_total_cost < 0 THEN
        SET v_amount_to_pay = 0;
        SET v_message = 'Lỗi: Chi phí không hợp lệ';
    -- Trường hợp hợp lệ
    ELSE
        SET v_message = 'Đã tính toán xong';
        
        IF p_patient_type = 'BHYT' THEN
            SET v_amount_to_pay = p_total_cost * 0.20;
        ELSEIF p_patient_type = 'VIP' THEN 
            SET v_amount_to_pay = p_total_cost * 0.90;
        ELSEIF p_patient_type = 'THUONG' THEN 
            SET v_amount_to_pay = p_total_cost;
        ELSE 
        -- Mặc định nếu gõ sai tên đối tượng
            SET v_amount_to_pay = p_total_cost;
        END IF;
        
        -- Cập nhật số tiền nợ vào hệ thống
        UPDATE Patient_Invoices
        SET total_due = v_amount_to_pay,
            last_updated = CURRENT_TIMESTAMP
        WHERE patient_id = p_patient_id;
    END IF;
    -- Kết quả
    SELECT v_amount_to_pay, v_message;
END //
DELIMITER ;

-- Kiểm thử đối tượng BHYT
-- Giảm 80%, chỉ đóng 20% của 100 = 20
CALL CalculatePatientInvoice(1, 'BHYT', 100.00);

-- Kiểm thử đối tượng VIP
-- Giảm 10%, phải đóng 90% của 100 = 90
CALL CalculatePatientInvoice(2, 'VIP', 100.00);

-- Kiểm thử đối tượng THUONG
-- Đóng đủ 100% = 100
CALL CalculatePatientInvoice(3, 'THUONG', 100.00);

-- Kiểm thử báo lỗi nhập tổng chi phí âm
-- Hệ thống chặn đứng, trả về số tiền phải thu là 0
CALL CalculatePatientInvoice(1, 'BHYT', -50.00);

SELECT patient_id, total_due, last_updated FROM Patient_Invoices;